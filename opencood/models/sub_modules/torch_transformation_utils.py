...
def warp_affine_expand(src, M, dsize,
        mode='bilinear',
        padding_mode='zeros',
        align_corners=False):

    new_src = torch.zeros(src.size()[0], src.size()[1], src.size()[2]*2, src.size()[3]*2)
    new_src[:,:, 128:384, 128:384] = src
    return new_src
    """
    B, C, H, W = new_src.size()
    grid = F.affine_grid(M,
                         [B, C, 512, 512],
                         align_corners=align_corners).to(new_src)
    return F.grid_sample(new_src, grid, align_corners=align_corners)
    """
