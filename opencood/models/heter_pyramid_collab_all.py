...
class HeterPyramidCollabAll(nn.Module):
    def __init__(self, args):
        super(HeterPyramidCollabAll, self).__init__()
        self.args = args
        ...
        for modality_name in self.modality_name_list:
            ...
            if sensor_name == "lidar" or "camera": #for allcross, only lidar will do BEV and following steps, camera only augment lidar
                """
                Backbone building
                """
            ...
            if sensor_name == "camera": #this mask part needs redesign?
    ...
    def forward(self, data_dict):
        ...
        affine_matrix = normalize_pairwise_tfm_2duplicate(data_dict['pairwise_t_matrix'], self.H, self.W, self.fake_voxel_size)
        #print(affine_matrix)
        ...
        for modality_name in self.modality_name_list: # was in agent_modality_list
            if len(modality_feature_dict[modality_name]) == 1:
                heter_feature_2d_list.append(modality_feature_dict[modality_name])
                print(str(modality_name))
            elif len(modality_feature_dict[modality_name]) > 1:
                #heter_feature_2d_list.append(modality_feature_dict[modality_name][0])
                for agent_i in range(len(modality_feature_dict[modality_name])):
                    heter_feature_2d_list.append(modality_feature_dict[modality_name][agent_i]) # was agent_i, all modaility and all agents together
                    print(str(modality_name)+str(agent_i))
        # measuring diff
        #diff = modality_feature_dict['m1']
        #print("differs at: " + str(torch.sum(diff)))
        # measure ends
        ...
        fused_feature, occ_outputs = self.pyramid_backbone.forward_collab(
                                                heter_feature_2d,
                                                record_len*2, #due to double modality total number, was record_len*2
                                                affine_matrix,
                                                agent_modality_list,
                                                self.cam_crop_info
                                            )
