...
from opencood.models.sub_modules.rfs_submodule import EfficientNetFeatureExtractor, ResNetFeatureExtractor
...
class LiftSplatShoot(nn.Module):
    ...
    def get_cam_feats(self, x):
        ...
        x = x.view(B*N, C, imH, imW)  # B和N两个维度合起来  x: 16 x 3 x 256 x 352
...
class RadianFeatureShoot(nn.Module):
    def __init__(self, args):
        super(RadianFeatureShoot, self).__init__()

        self.downsample = args['img_downsample']  # should be useless, we donnot apply downsampling here
        self.camC = args['img_features']  # 图像特征维度
        target_H, target_W = 144, 256
        self.camera_encoder_type = args['camera_encoder']
        if self.camera_encoder_type == 'EfficientNet':
            self.camencode = EfficientNetFeatureExtractor(self.camC, target_H, target_W)
        elif self.camera_encoder_type == 'Resnet101':
            self.camencode = ResNetFeatureExtractor(self.camC, target_H, target_W)

    def forward(self, data_dict, modality_name):
        # x: [N,n,3,H,W] N for num of agents, n for cameras per agent, 3 for RGB, remaining 2 are H W sizes
        # the config of N, n, H, W depends on dataset (OpenV2V, DAIR-V2X)
        # rots: [N,n,3,3]
        # trans: [N,n,3]
        # intrins: [N,n,3,3]
        # post_rots: [N,n,3,3]
