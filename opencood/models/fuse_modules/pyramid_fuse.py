...
def weighted_fuse(x, score, record_len, affine_matrix, align_corners):
    ...
    for b in range(B):
        ...
        if H == 0:
            print("t_matrix is:")
            print(t_matrix[i])
            gray_image_function(batch_node_features[b][1], "debug_feature_map/outer")
            gray_image_function(feature_in_ego[1], "debug_feature_map/transformed_outer_simple")
            gray_image_function(batch_node_features[b][0], "debug_feature_map/ego")
            #gray_image_function(feature_in_ego[0], "debug_feature_map/transformed_ego_simple")
        """
        if C == 64:
            color_image_function(feature_in_ego[0], "debug_feature_map/ego")
            color_image_function(feature_in_ego[1], "debug_feature_map/outer_on_ego")
        """
        ...
        scores_in_ego = torch.where(torch.isnan(scores_in_ego),
                                    ...
                                    scores_in_ego)
        # please do not use following code for full fusion, only for selecting specifc agent-feature agent-socre tensor:
        """
        #feature_in_ego = torch.cat([feature_in_ego[0:1], feature_in_ego[2:3]], dim=0)
        #scores_in_ego = torch.cat([scores_in_ego[0:1], scores_in_ego[2:3]], dim=0)
        feature_in_ego = feature_in_ego[0:1]
        scores_in_ego = scores_in_ego[0:1]
        """
        """
        temp = torch.sum(feature_in_ego * scores_in_ego, dim=0)
        if temp.shape[0] == 64:
            color_image_function(temp, "debug_feature_map/fused")
        """
...
class PyramidFusion(ResNetBEVBackbone):
    ...
    def forward_collab(self, spatial_features, record_len, affine_matrix, agent_modality_list = None, cam_crop_info = None):
        ...
        for i in range(self.num_levels):
            ...
            #print(str(i) + "level occ_map size: ")
            #print(occ_map.size())
