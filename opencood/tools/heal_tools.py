...
def get_sync_model_path_from_dir(model_dir, sync_num):
    sync_name = '*epoch' + str(sync_num) + '*.pth'
    file_list = glob.glob(os.path.join(model_dir, sync_name))
    if len(file_list):
        assert len(file_list) == 1
        model_path = file_list[0]
    else:
...
def merge_dict(single_model_dict, stage1_model_dict):
    ...
    m1_overlap_set = {value for value in symm_diff_set if 'm1' in value}
    print(len(m1_overlap_set))
    ...
    for key in single_model_dict:
        ...
        if 'layers_m' in key or 'head_m' in key or 'shrink_conv_m' in key:
            print(f"Pass {key}")
            continue
        if key in symm_diff_set and "m1" in key:
            print("Rename key to avoid overriding parameters due to same modality names, only effective for merginng 2 m1s!")
            merged_dict[key.replace("m1", "m5")] = single_model_dict[key] # keep existing modules
        else:
            merged_dict[key] = single_model_dict[key] # keep existing modules

    for key in stage1_keys:
        merged_dict[key] = stage1_model_dict[key] # update new modules from sequenced model
...
def sync_merge_and_save_final(aligned_model_dir_list, output_model_dir, sync_num):
    """
    will refer the base model's best eval as sync epoch number
    """
    final_dict = OrderedDict()

    for i, aligned_model_dir in enumerate(aligned_model_dir_list):
        if i == len(aligned_model_dir_list) - 1:
            aligned_model_path = get_model_path_from_dir(aligned_model_dir)
        if i < len(aligned_model_dir_list) - 1:
            aligned_model_path = get_sync_model_path_from_dir(aligned_model_dir, sync_num)
        model_dict = torch.load(aligned_model_path, map_location='cpu')
        final_dict = merge_dict(final_dict, model_dict)
...
if __name__ == "__main__":
    ...
    elif func == 'sync_merge_final':
        sync_merge_and_save_final(sys.argv[2:-2], sys.argv[-2], int(sys.argv[-1]))
    else:
