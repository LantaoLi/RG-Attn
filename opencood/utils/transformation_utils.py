        ...
        for i in range(len(t_list)):
            for j in range(len(t_list)):
                ...
                if i != j:
                    ...
                    #pairwise_t_matrix[i, j] = t_matrix #original
                    pairwise_t_matrix[i, j] = t_matrix #swap i and j here would lead to transformation matrix flip over
...
def normalize_pairwise_tfm_2duplicate(pairwise_t_matrix, H, W, discrete_ratio, downsample_rate=1):
    """
    normalize the pairwise transformation matrix to affine matrix need by torch.nn.functional.affine_grid()
    Args:
        pairwise_t_matrix: torch.tensor
    ...
    affine_matrix[...,0,2] = affine_matrix[...,0,2] / (downsample_rate * discrete_ratio * W) * 2
    affine_matrix[...,1,2] = affine_matrix[...,1,2] / (downsample_rate * discrete_ratio * H) * 2
    expanded_affine_matrix = torch.cat((affine_matrix, affine_matrix), dim=2)
    return expanded_affine_matrix
...
def tfm_to_pose(tfm: np.ndarray):
    ...
    yaw = np.degrees(np.arctan2(tfm[1,0], tfm[0,0])) # tryout for dair-v2x
...
def x1_to_x2(x1, x2):
    ...
    transformation_matrix = np.dot(world_to_x2, x1_to_world) # T2w * Tw1 = T21, here something wrong
    """
    x1_to_world = x_to_world(x1) # wP = x1_to_world * 1P, so x1_to_world is Tw1
    x2_to_world = x_to_world(x2) # Tw2
    world_to_x2 = np.linalg.inv(x2_to_world) # T2w
        ...
        for i in range(len(t_list)):
            for j in range(len(t_list)):
                ...
                if i != j:
                    ...
                    # pairwise_t_matrix[b][i, j] = t_matrix #original
...
def muilt_coord(rotationA2B, translationA2B, rotationB2C, translationB2C):
    ...
    rotation = np.dot(rotationB2C, rotationA2B)#questionable
    #rotation = np.dot(rotationA2B, rotationB2C)
