...
class MAVoxelPostprocessor(BasePostprocessor):
    def __init__(self, anchor_params, train):
        super(MAVoxelPostprocessor, self).__init__(anchor_params, train)
        self.anchor_num_perlevel = int(self.params['anchor_args']['num']/len(self.params['anchor_args']['l']))
        self.anchor_num = int(self.params['anchor_args']['num'])
    ...
    def generate_anchor_box(self):
        ...
        assert self.anchor_num_perlevel == len(r)
        ...
        cx = np.tile(cx[..., np.newaxis], self.anchor_num_perlevel) # center
        cy = np.tile(cy[..., np.newaxis], self.anchor_num_perlevel)
        cz = np.ones_like(cx) * -1.0

        r_ = np.ones_like(cx)
        for i in range(self.anchor_num_perlevel):
            r_[..., i] = r[i]

        anchors_list = []
        for i in range(len(l)):
            l_tile = np.ones_like(cx)*l[i]
            w_tile = np.ones_like(cx)*w[i]
            h_tile = np.ones_like(cx)*h[i]

            if self.params['order'] == 'hwl': # pointpillar
                anchors = np.stack([cx, cy, cz, h_tile, w_tile, l_tile, r_], axis=-1) # (50, 176, 2, 7)
            elif self.params['order'] == 'lhw':
                anchors = np.stack([cx, cy, cz, l_tile, h_tile, w_tile, r_], axis=-1)
            else:
                sys.exit('Unknown bbx order.')
            anchors_list.append(anchors)

        all_anchors = np.concatenate(anchors_list, axis=2)
        #print(anchors)
        return all_anchors
    ...
    def generate_label(self, **kwargs):
        ...
        # (H, W, 2)
        pos_equal_one = np.zeros((*feature_map_shape, num_anchors))
        neg_equal_one = np.zeros((*feature_map_shape, num_anchors))
        # (H, W, num_anchors * 7)
        targets = np.zeros((*feature_map_shape, num_anchors * 7))
        ...
        index_x, index_y, index_z = np.unravel_index(
            id_pos, (*feature_map_shape, num_anchors))
        ...
        index_x, index_y, index_z = np.unravel_index(
            id_neg, (*feature_map_shape, num_anchors))
        neg_equal_one[index_x, index_y, index_z] = 1

        # to avoid a box be pos/neg in the same time
        index_x, index_y, index_z = np.unravel_index(
            id_highest, (*feature_map_shape, num_anchors))
    ...
    def post_process(self, data_dict, output_dict):
        ...
        # remove large bbx
        #keep_index_1 = box_utils.remove_large_pred_bbx(pred_box3d_tensor) #original code, why not keep large predicted ones?
        keep_index_1 = box_utils.remove_bbx_abnormal_z(pred_box3d_tensor) #to replace large remove, we duplicate z abnormal instead
