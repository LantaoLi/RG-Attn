...
intermediate heterall fusion dataset, all modaility data will be processed and retained
...
def getIntermediateheterallFusionDataset(cls):
    """
    cls: the Basedataset.
    """
    class IntermediateheterallFusionDataset(cls):
        def __init__(self, params, visualize, train=True):
            ...
            self.anchor_box_torch = torch.from_numpy(self.anchor_box) # previous code
            ...
            # print("Assignment_path is", params['heter']['assignment_path'])
            ...
            self.sensor_type_dict['m1&m2'] = 'lidar_camera'
        ...
        def get_item_single_car(self, selected_cav_base, ego_cav_base):
            ...
            # lidar
            if 'lidar' in sensor_type or self.visualize:
                # process lidar
                lidar_modality_name = 'm1' #please make m1 to lidar in this case
                ...
                #print("transformation_matrix for raw lidar np:")
                ...
                if self.kd_flag:
                    ...
                    processed_lidar_proj = eval(f"self.pre_processor_{lidar_modality_name}").preprocess(lidar_proj_np)
                    selected_cav_processed.update({f'processed_features_{lidar_modality_name}_proj': processed_lidar_proj})

                if 'lidar' in sensor_type:
                    processed_lidar = eval(f"self.pre_processor_{lidar_modality_name}").preprocess(lidar_np)
                    selected_cav_processed.update({f'processed_features_{lidar_modality_name}': processed_lidar})
            ...
            if 'camera' in sensor_type:
                ...
                for idx, img in enumerate(camera_data_list):
                    ...
                    resize, resize_dims, crop, flip, rotate = sample_augmentation(
                        eval(f"self.data_aug_conf_{camera_modality_name}"), self.train
                    )
                ...
                selected_cav_processed.update(
                    {
                    f"image_inputs_{camera_modality_name}":
                        {
        ...
        def __getitem__(self, idx):
            ...
            for cav_id, selected_cav_base in base_data_dict.items():
                ...
                if self.adaptor.unmatched_modality(selected_cav_base['modality_name']):
                    #exclude_agent.append(cav_id)
                    print("unmatched modality here in dataset processing heterall!")
                    continue
                """
            ...
            for _i, cav_id in enumerate(cav_id_list):
                ...
                if not self.visualize and sensor_type == "camera": #only when camera is the only modality
                ...
                if sensor_type == "lidar":
                    ...
                    eval(f"input_list_m1").append(selected_cav_processed[f"processed_features_m1"])
                    eval(f"input_list_m2").append(selected_cav_processed[f"image_inputs_m2"])
                    agent_modality_list.append('m1')
                    agent_modality_list.append('m2')
                else:
                    raise

                #agent_modality_list.append(modality_name) #forwarded this function

                if self.visualize or self.kd_flag:
                    # heterogeneous setting do not support disconet' kd
                    projected_lidar_stack.append(
                        selected_cav_processed['projected_lidar'])
                    if "lidar" in sensor_type and self.kd_flag:
                        eval(f"input_list_m1_proj").append(selected_cav_processed[f"processed_features_m1_proj"])
            ...
            #processed_data_dict['ego'].update({'cav_id_list': cav_id_list})
        ...
        def collate_batch_train(self, batch):
            ...
            for i in range(len(batch)):
                if batch[i] is None:
                    print('batch ' + str(i) + ' is none!')
                if batch[i] is None:
                    print('batch ' + str(i) + 'ego is none!')
                ...
                #print("update pairwise_t_matrix_list:")
                #print(ego_dict['pairwise_t_matrix'])
            ...
            output_dict['ego'].update({"sample_idx": batch[0]['ego']['sample_idx']}) #newly added codes for debug in training
    ...
    return IntermediateheterallFusionDataset
