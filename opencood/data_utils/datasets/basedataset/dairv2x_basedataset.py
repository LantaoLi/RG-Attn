...
class DAIRV2XBaseDataset(Dataset):
    ...
    def retrieve_base_data(self, idx):
        ...
        #print("idx: " + str(idx) + "and vehicle frame id: " + str(veh_frame_id))
        frame_info = self.co_data[veh_frame_id]
        #print("frame info: " + str(frame_info))
        ...
        #print(data[0]['params']['lidar_pose'])
        ...
        #print(data[1]['params']['lidar_pose'])
        ...
        if getattr(self, "heterogeneous", False):
            ...
            if "fixed_ego" in self.params and "reverse_ego" in self.params and self.params['reverse_ego'] == True:
                print("reversed ego")
                data[0], data[1] = data[1], data[0]
                data[0]['ego'] = True
                data[1]['ego'] = False

            if self.train and "fixed_ego" not in self.params: # randomly choose RSU or Veh to be Ego
                p = np.random.rand()
                if p > 0.5:
                    print("ego changed in train!")
                    data[0], data[1] = data[1], data[0]
                    data[0]['ego'] = True
                    data[1]['ego'] = False
            else:
            ...
            if "all_modality" in self.params and self.params['all_modality'] == True:
                data[0]['modality_name'] = 'm1&m2'
                data[1]['modality_name'] = 'm1&m2'
        return data
